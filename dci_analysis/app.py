...
dashboard.layout = html.Div(
    ...
    children=[
        ...
        html.Div(id="container_1", children=[html.H1("DCI Analysis")]),
        html.Div(
            id="container_2",
            children=[
                html.Div(
                    ...
                    children=[
                        html.Div(
                            ...
                            children=[
                                ...
                                dcc.Dropdown(
                                    ...
                                    options=[
                                        {
                                            "label": "RHEL-7.6-milestone",
                                            "value": "RHEL-7.6-milestone",
                                        },
                                        {
                                            "label": "RHEL-7.7-milestone",
                                            "value": "RHEL-7.7-milestone",
                                        },
                                        {
                                            "label": "RHEL-7.8-milestone",
                                            "value": "RHEL-7.8-milestone",
                                        },
                                        {
                                            "label": "RHEL-7.9-milestone",
                                            "value": "RHEL-7.9-milestone",
                                        },
                                        ...
                                        {
                                            "label": "RHEL-8.0-milestone",
                                            "value": "RHEL-8.0-milestone",
                                        },
                                        {
                                            "label": "RHEL-8.1-milestone",
                                            "value": "RHEL-8.1-milestone",
                                        },
                                        {
                                            "label": "RHEL-8.2-milestone",
                                            "value": "RHEL-8.2-milestone",
                                        },
                                        {
                                            "label": "RHEL-8.3-milestone",
                                ...
                                html.Label("Tags"),
                                dcc.Input(
                                    id="topic_1_tags", value="x86_64", debounce=True
                                ),
                                html.Br(),
                                html.Div(dcc.DatePickerRange(id="topic_1_timeframe")),
                                html.Br(),
                                dcc.RadioItems(
                                ...
                                dcc.Input(
                                    id="cov_filtration", value="0.05", debounce=True
                                ),
                                html.Br(),
                            ],
                        ),
                        ...
                        html.Div(
                            ...
                            children=[
                                ...
                                html.Label("Tags"),
                                dcc.Input(
                                    id="topic_2_tags", value="x86_64", debounce=True
                                ),
                                html.Br(),
                                html.Div(dcc.DatePickerRange(id="topic_2_timeframe")),
                                html.Br(),
                                dcc.RadioItems(
...
def update_output(
    ...
    if n_clicks == 0:
        return (
            "Compute the overview comparison between topics !",
        ...
        def get_coefficient_variation_table(
            topic_name, topic_2_start_date, topic_2_end_date, topic_tags, threshold=None
        ):
            jobs, _ = analyzer.get_jobs_dataset(
                topic_name, topic_2_start_date, topic_2_end_date, topic_tags
            )
            ...
            for testcase in testcases:
                if not threshold:
                    data.append({"testcase": testcase, "value": coeff_var[testcase]})
        ...
        filtered_tests = _filter_tests_by_cov(
            coefficient_variations_table_1.data, cov_filtration
        )
        ...
        comparisons = dcc.Graph(
            figure={
                ...
                "layout": {
                    "title": "Baseline %s/%s vs %s/%s"
                    % (topic_1, topic_1_computation, topic_2, topic_2_computation),
                    "xaxis": {
        ...
        for testcase in testcases:
            data.append(
                {"testcase": testcase, "value": compared_jobs.loc[testcase, job_id]}
            )
        ...
        def graph_per_class(topic, topic_start_date, topic_end_date, topic_tags):
            ...
            jobs, jobs_ids_dates = analyzer.get_jobs_dataset(
                topic,
                topic_start_date,
                topic_end_date,
                topic_tags,
            ...
            fig = make_subplots(rows=nb_rows, cols=1, subplot_titles=jobs_index)
            ...
            for row, j_i in enumerate(jobs_index, 1):
                ...
                fig.append_trace(
                    go.Scatter(
                        x=j_c,
                        text=jobs_columns,
                        y=list(jobs_sum_per_class.loc[j_i]),
                        name="lol",
                    ),
                    row=row,
                    col=1,
        ...
        graph_per_class_topic_1 = graph_per_class(
            topic_1, topic_1_start_date, topic_1_end_date, topic_1_tags
        )
        graph_per_class_topic_2 = graph_per_class(
        ...
        trends = dcc.Graph(
            figure={
                ...
                "layout": {
                    "title": "Evolution of the %sth tests, %s/%s vs %s/%s"
                    % (
                        evolution_percentage_value,
                        topic_1,
                        topic_1_computation,
                    ...
                    "xaxis": {"title": "%s/Jobs" % topic_1_computation},
                    "yaxis": {"title": "Maximum evolution of 95% tests"},
                },
            }
        )
        ...
        for jid in jobs_ids_dates:
            data.append(
                {
                    "date": jid["date"],
                    "id": "[%s](https://www.distributed-ci.io/jobs/%s/jobStates)"
                    % (jid["id"], jid["id"]),
                }
            )
        ...
        return (
            comparisons,
            comparisons_details,
            coefficient_variations_table_1,
            coefficient_variations_table_2,
            graph_per_class_topic_1,
            graph_per_class_topic_2,
            trends,
            trends_jobs_details,
        )
...
def get_min_max_date_from_topic(topic_name):
    ...
    if len(sorted_csv_files) >= 2:
        ...
        max_date = analyzer.string_to_date(
            max_file.split("/")[1].split("_")[0]
        ) + timedelta(days=1)
...
@dashboard.callback(
    [
        dash.dependencies.Output("topic_1_timeframe", "min_date_allowed"),
...
@dashboard.callback(
    [
        dash.dependencies.Output("topic_2_timeframe", "min_date_allowed"),
